...
import 'package:kontrol_app/presentation/views/forms/final_form.dart';
...
class FormTwoStepView extends ConsumerStatefulWidget {
  final TechnicalControl? preloadedData;

  const FormTwoStepView({super.key, this.preloadedData});
...
class _FormTwoStepViewState extends ConsumerState<FormTwoStepView> {
  ...
  DateTime? movementDateTimeFinal;
  final TextEditingController _movementDateControllerFinal = TextEditingController();
  ...
  int? selectedFuel;
  int? selectedFuelFinal;
  List<File?> photoFiles = [];
  List<File?> photoFilesFinal = [];
  ...
  void initState() {
    super.initState();
    // _formKeys = List.generate(formLen, (_) => GlobalKey<FormState>());
    // ⚠️ En Riverpod las llamadas async se hacen en microtask
    Future.microtask(() async {
      await _loadDrivers();
      await _loadProjects();
      await _loadLicenses();
      await _loadLevelGasoline();
      await _loadReasons();
      await _loadCopilot();

      // Si hay datos pre-cargados, cargarlos después de que las opciones estén disponibles
  ...
  Future<void> _pickDateTimeFinal() async {
    final date = await showDatePicker(
      context: context,
      initialDate: movementDateTimeFinal ?? DateTime.now(),
      firstDate: DateTime(2000),
      lastDate: DateTime(2100),
    );
    if (date == null) return;
    final time = await showTimePicker(
      context: context,
      initialTime: TimeOfDay.fromDateTime(movementDateTimeFinal ?? DateTime.now()),
    );
    if (time == null) return;
    setState(() {
      movementDateTimeFinal = DateTime(
        date.year,
        date.month,
        date.day,
        time.hour,
      ...
      _movementDateControllerFinal.text = movementDateTimeFinal.toString();
    });
  }
  ...
  Future<void> _pickImagesFinal() async {
    final result = await FilePicker.platform.pickFiles(
      allowMultiple: true,
      type: FileType.image,
    );
  ...
  void _removeImageFinal(int index) {
    setState(() {
      photoFilesFinal.removeAt(index);
    });
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final request = TechnicalRequest(
        ...
        projectOther: _projectOtherController.text.isEmpty
            ? null
            : _projectOtherController.text,
        plate: selectedPlate!,
      ...
      if (mounted) {
        ...
        if (e.response?.statusCode == 400) {
          errorMessage =
              'Datos inválidos: ${e.response?.data['message'] ?? ''}';
  ...
  void _resetForm() {
    setState(() {
      ...
      movementDateTimeFinal = null;
      _movementDateControllerFinal.clear();
      selectedFuelFinal = null;
      photoFilesFinal.clear();
    });
  }

  void _loadPreloadedData(TechnicalControl data) {
    if (!mounted) return;
    ...
    setState(() {
      ...
      selectedProjectsIds = data.clients.map((r) => r.id).toList();
      selectedProjectsNames = data.clients.map((r) => r.name).toList();

      // Cargar motivos
      selectedMotivesIds = data.reasons.map((r) => r.id).toList();
      selectedMotivesNames = data.reasons.map((r) => r.name).toList();

      // Cargar placa
      selectedPlate = data.licenseId;
      ...
      // Cargar copilotos
      selectedCopilotIds = data.copilots.map((c) => c.id).toList();
      selectedCopilotNames = data.copilots.map((c) => c.name).toList();
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.preloadedData != null) {
      stepSubtitles.add('Cierre');
      stepTitles.add('Cierre movimiento');
      finalStep = 4;
    }
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: _isLoading
                          ? null
                          : (_currentStep == finalStep ? _submit : _nextStep),
                      ...
                      child: _isLoading
                          ...
                          : Text(
                              _currentStep == finalStep ? 'ENVIAR' : 'SIGUIENTE',
                      ...
                      child: OutlinedButton(
                        ...
                        style: OutlinedButton.styleFrom(
                          side: const BorderSide(
                            color: Color.fromARGB(255, 4, 98, 246),
                            width: 2,
                          ),
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(8),
    ...
    return Row(
      ...
      children: List.generate(subtitles.length, (index) {
        ...
        final color = isActive || isCompleted
            ? const Color.fromARGB(255, 4, 98, 246)
            : Colors.grey[300];
  ...
  Widget _buildFormContent() {
    final forms = [
      ...
      if (widget.preloadedData != null)
        ...
        key: _formKeys[4],
        child: FinalForm(
          movementDateController: _movementDateControllerFinal,
          onPickDateTime: _pickDateTimeFinal,
          selectedFuel: selectedFuelFinal,
          fuelOptions: fuelOptions,
          onFuelChanged: (v) => setState(() => selectedFuelFinal = v),
          detailsController: _detailsController,
          onRemoveImage: _removeImageFinal,
          selectedImages: photoFilesFinal,
          onPickImages: _pickImagesFinal,
        ),
      ),
    ];
